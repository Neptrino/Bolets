...
| `import-xema-rain` | Hourly at minute 50 | Imports Meteocat XEMA semi-hourly gauge precipitation collapsed to station hours with completeness counts, keeping a 60-day rolling window. These hours feed the promoted `station-rain-v1` past-precipitation correction in the spatial refresh. `npm run weather:backfill-xema-rain` refills the window through the same function; `npm run weather:compare-station-rain` re-runs the station-versus-model validation. |
