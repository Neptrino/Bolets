-- A manual observation refresh may reset the observed cursors before both
-- streams finish. Preserve the completed forecast cursor during that window so
-- the resumable writer cannot overwrite an already published issue batch by
-- batch. Once both observations complete, the same RPC either keeps the issue
-- or invalidates it for a clean rebuild.
...
comment on function public.reconcile_weather_forecast_issue(date, interval) is
  'Preserves a completed issue while observations refresh, then invalidates it only when both observed streams have crossed the safe calibration seam.';
