...
begin
  ...
  order by coverage desc, levels.cell_id
  limit least(greatest(p_limit, 1), 1000);
end;
$$;
...
comment on function public.read_potential_habitat_cells(
  double precision, double precision, double precision, double precision,
  integer, text[], double precision, double precision, double precision, double precision, integer
) is 'Plans exact and coarse habitat reads independently so large-area coverage uses a selective plan and retains exact 250 m ecological gates.';
