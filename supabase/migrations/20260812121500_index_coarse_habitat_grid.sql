create index if not exists spatial_cells_grid_xy_idx
...
as $$
  ...
  visible_grid_extent as materialized (
    select
      min(levels.bucket_x) * (p_grid_size_m / 250) as min_x,
      (max(levels.bucket_x) + 1) * (p_grid_size_m / 250) - 1 as max_x,
      min(levels.bucket_y) * (p_grid_size_m / 250) as min_y,
      (max(levels.bucket_y) + 1) * (p_grid_size_m / 250) - 1 as max_y
  ...
  visible_base as materialized (
    ...
    cross join visible_grid_extent extent
    where cells.static_verified
      and extent.min_x is not null
      and pg_catalog.split_part(cells.cell_id, ':', 3)::integer between extent.min_x and extent.max_x
      and pg_catalog.split_part(cells.cell_id, ':', 4)::integer between extent.min_y and extent.max_y
  ),
  ...
  coarse_grouped as materialized (
    ...
    join visible_levels levels on levels.bucket_x =
