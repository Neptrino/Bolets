-- Extend the daily forecast with best-effort outlook horizons (168-336 h),
-- serving the slow-guild matured-rain window whose content is already fully
-- observed up to 14 days ahead. The five-day core keeps its all-or-nothing
-- publication contract, so every completeness count now measures core
-- horizons (<= 120 h) only: a missing hour at day 14 must never block the
-- daily forecast. Outlook rows are stored only when complete and read
-- opportunistically.
...
begin
  ...
  where snapshot_date = p_snapshot_date
    ...
    -- Outlook horizons (> 120 h) are best-effort and excluded from the
    -- completion contract.
    and horizon_hours <= 120;
...
as $$
  with product_models(model, stream, cursor_pipeline) as (
    ...
    select
      ...
      issue.generated_at,
      issue.completed_at,
      count(forecast.point_id) filter (where forecast.horizon_hours <= 120)::integer as row_count,
      count(distinct forecast.point_id)::integer as point_count,
      expected.point_count as expected_point_count,
      count(distinct forecast.horizon_hours) filter (where forecast.horizon_hours <= 120)::integer as horizon_count,
      count(distinct forecast.horizon_hours)
        filter (where forecast.horizon_hours > 0 and forecast.horizon_hours <= 120)::integer as future_horizon_count,
      ...
      (
        ...
        and count(forecast.point_id) filter (where forecast.horizon_hours <= 120) = expected.point_count * 6
        and count(distinct forecast.point_id) = expected.point_count
        and count(distinct forecast.horizon_hours) filter (where forecast.horizon_hours <= 120) = 6
      ) as complete
