-- Preserve the trailing seven-day air-humidity mean across live 1 km
-- aggregation and the cached coarse condition levels used by map scoring.
...
comment on function public.read_aggregated_cell_environment(double precision, double precision, double precision, double precision, integer, integer) is
  'Returns zoom-adaptive conditions with trailing 10-day temperature, 7-day air-humidity, and 30-day antecedent moisture context preserved from normalized weather snapshots.';

comment on function public.refresh_spatial_level_conditions(integer, date) is
  'Refreshes coarse current-condition caches while preserving trailing 10-day temperature, 7-day air-humidity, and 30-day antecedent moisture context.';
