-- Grass-mosaic cover for grassland species. Fairy-ring meadows in the
-- Pyrenean foothills are grazed pastures with scattered shrubs, which the
-- cover source classifies mostly as "matollars": the abundance-graded 2026-05
-- Marasmius days scored O 32-36 purely because their cells carried 4 of 25
-- grass samples while the fruiting conditions scored 66-87. For species whose
-- vocabulary declares grassland (the c228 bucket) and does not already match
-- the scrub bucket (c224), matollar samples now count at 0.4 weight when the
-- cell holds at least 2 real grass samples: a shrubby pasture scores like the
-- mosaic it is, while pure scrub with no grass stays incompatible. The rule
-- adds no newly compatible cells (grass >= 2 already passed the gate); it
-- raises coverage on existing grass cells (sampled mean 0.30 -> 0.37).
-- Cross-set validated 2026-09-09: findings hit40 0.38 -> 0.42 with AUC flat,
-- GBIF unchanged; the four graded Marasmius days rise to O 41-62.
...
as $$
  ...
  coarse_grouped as materialized (
    ...
    cross join lateral (
      ...
      + case when compatibility.c228 and not compatibility.c224
          and ((cells.habitat_cover_counts >> 35) & 31::bigint) >= 2
        then 0.4 * ((cells.habitat_cover_counts >> 15) & 31::bigint)::double precision
...
begin
  ...
  scored_base as materialized (
    ...
    cross join lateral (
      ...
      + case when array['prats', 'pastures', 'gespes', 'vores de cami', 'clarianes', 'vores de bosc'] && profiles.forest_terms
          and not (array['matollars', 'clarianes', 'vores de bosc'] && profiles.forest_terms)
          and ((cells.habitat_cover_counts >> 35) & 31::bigint) >= 2
        then 0.4 * ((cells.habitat_cover_counts >> 15) & 31::bigint)::double precision
        else 0::double precision end
      ) / 25), profiles.forest_terms)
