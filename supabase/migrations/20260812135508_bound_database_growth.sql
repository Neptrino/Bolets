...
begin
  ...
  begin
    ...
    -- Each weather row already contains every rolling input used by prediction.
    -- Keeping today and yesterday preserves the latest snapshot and 36-hour
    -- fallback while preventing this append-only table from exhausting storage.
    delete from public.weather_grid_snapshots
    ...
    -- Active pg_cron runs have no end_time, so they are never removed here.
...
-- A named pg_cron schedule is updated in place when the name already exists.
-- Keeping the cron command short also stops duplicating a multi-statement SQL
-- payload in every cron.job_run_details row.
