-- Companion to 20260816090000_taper_habitat_soil_ph: apply the same soil-pH
-- taper to the exact 250 m branch of the weighted habitat reader. The pH
-- weight multiplies cover_score instead of filtering rows, so a cell just
-- outside a species' pH window contributes reduced coverage rather than
-- disappearing, and altitude_weighted_coverage stays <= coverage. Cells with
-- a zero pH weight still drop out through the existing cover_score > 0 gate.
