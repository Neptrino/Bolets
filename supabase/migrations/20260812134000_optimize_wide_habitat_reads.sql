-- Wide prediction views can touch more than 150,000 exact habitat cells. Keep
-- that scan inside one compact covering index and apply the ecological gates
-- before aggregation, rather than constructing a large array of text IDs and
-- probing the wide spatial_cells heap once per child.
create index if not exists spatial_cells_habitat_grid_covering_idx
  on public.spatial_cells (
    ((pg_catalog.split_part(cell_id, ':', 4))::integer),
    ((pg_catalog.split_part(cell_id, ':', 3))::integer)
  )
  include (habitat_cover_counts, habitat_altitude_m, habitat_soil_ph)
