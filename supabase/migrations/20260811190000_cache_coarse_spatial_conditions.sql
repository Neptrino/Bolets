alter table public.spatial_cell_levels
  add column if not exists condition_snapshot_date date,
  add column if not exists condition_observed_at timestamptz,
  add column if not exists condition_sources text[],
  add column if not exists condition_source_resolution_m integer,
  add column if not exists condition_confidence text,
  add column if not exists condition_stale boolean,
  add column if not exists condition_unavailable_fields text[],
  add column if not exists condition_values jsonb;

alter table public.spatial_cell_levels
  drop constraint if exists spatial_cell_levels_condition_confidence_check;

alter table public.spatial_cell_levels
  add constraint spatial_cell_levels_condition_confidence_check
  check (condition_confidence is null or condition_confidence in ('high', 'moderate', 'limited', 'unknown'));
...
as $$
  ...
  select
    ...
    candidates.east,
    candidates.north,
    candidates.grid_size_m,
    candidates.condition_observed_at,
    candidates.condition_sources,
    candidates.condition_source_resolution_m,
    candidates.condition_confidence,
    candidates.condition_stale or candidates.condition_observed_at < now() - interval '36 hours',
    candidates.condition_unavailable_fields,
    candidates.condition_values
  from candidates
  order by
    power(((candidates.west + candidates.east) / 2) - ((p_west + p_east) / 2), 2)
    + power(((candidates.south + candidates.north) / 2) - ((p_south + p_north) / 2), 2)
...
revoke all on function public.read_precomputed_cell_environment(double precision, double precision, double precision, double precision, integer, integer) from public, anon, authenticated;
grant execute on function public.read_precomputed_cell_environment(double precision, double precision, double precision, double precision, integer, integer) to service_role;

comment on function public.refresh_spatial_level_conditions(integer, date) is
  'Refreshes the current environmental cache for one coarse map level from normalized weather snapshots.';

comment on function public.read_precomputed_cell_environment(double precision, double precision, double precision, double precision, integer, integer) is
  'Reads precomputed coarse environmental cells without aggregating provider data during map interaction.';
