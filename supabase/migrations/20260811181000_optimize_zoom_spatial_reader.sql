...
as $$
  ...
  atmosphere_values as (
    ...
    from atmosphere_links links
    join atmosphere_latest latest using (point_id)
    group by links.cell_id
  ),
  atmosphere_sources as (
    select links.cell_id, array_agg(distinct source_name order by source_name) as sources
    from atmosphere_links links
    join atmosphere_latest latest using (point_id)
    cross join lateral unnest(latest.sources) source_value(source_name)
    group by links.cell_id
  ),
  atmosphere_unavailable as (
    select links.cell_id, array_agg(distinct field_name order by field_name) as unavailable_fields
    from atmosphere_links links
    join atmosphere_latest latest using (point_id)
    cross join lateral unnest(latest.unavailable_fields) unavailable_value(field_name)
    group by links.cell_id
  ),
  atmosphere as (
    select values.*, coalesce(sources.sources, '{}'::text[]) as sources, coalesce(unavailable.unavailable_fields, '{}'::text[]) as unavailable_fields
    from atmosphere_values values
    left join atmosphere_sources sources using (cell_id)
    left join atmosphere_unavailable unavailable using (cell_id)
  ),
  soil_links as materialized (
    select levels.cell_id, point_id
    from visible_levels levels
    cross join lateral unnest(levels.soil_point_ids) soil_point(point_id)
  ),
  soil_latest as materialized (
    select distinct on (snapshots.point_id) snapshots.*
    from public.weather_grid_snapshots snapshots
    join (select distinct point_id from soil_links) links using (point_id)
    order by snapshots.point_id, snapshots.snapshot_date desc
  ),
  soil_values as (
    select
    ...
    from soil_links links
    join soil_latest latest using (point_id)
    group by links.cell_id
  ),
  soil_sources as (
    select links.cell_id, array_agg(distinct source_name order by source_name) as sources
  ...
  soil as (
    select values.*, coalesce(sources.sources, '{}'::text[]) as sources, coalesce(unavailable.unavailable_fields, '{}'::text[]) as unavailable_fields
    from soil_values values
    left join soil_sources sources using (cell_id)
    left join soil_unavailable unavailable using (cell_id)
  )
  select
    levels.cell_id,
    levels.region_id,
