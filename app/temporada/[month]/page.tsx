...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = pageTitle(`Bolets ${monthWithPreposition(month)}: espècies i calendari`);
  const description = metaDescription(`Consulta les ${activeSpecies.length} espècies de bolets actives ${monthWithPreposition(month)} a Catalunya, amb hàbitat, identificació i calendari ecològic mensual.`);
