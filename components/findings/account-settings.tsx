...
export function AccountSettings({ email }: { email: string }) {
  ...
  return <div className="finding-stack account-content">
    <section className="card finding-account-card finding-stack"><h2>Identitat pública</h2><p>Compte: <strong>{email}</strong>. El correu mai no es publica.</p><form className="finding-stack" onSubmit={save}><label className="finding-field">Àlies opcional<input minLength={3} maxLength={30} value={alias} onChange={(event) => setAlias(event.target.value)} placeholder="Per exemple, BoscEndins" /><small>Encara que tinguis àlies, decideixes troballa per troballa si es mostra.</small></label><button className="pill finding-button" disabled={busy}>Desar l’àlies</button></form>{message ? <p className="notice finding-notice">{message}</p> : null}</section>
    <PasskeySettings />
    <section className="card finding-account-card finding-stack"><h2>Sessió</h2><button className="pill finding-button-secondary" onClick={() => void signOut()}>Tancar sessió a tots els dispositius</button></section>
    <section className="card finding-account-card finding-stack"><h2>Eliminar el compte</h2><p>S’esborren el punt exacte, les notes, les fotos i les troballes privades. Les aportacions que ja eren públiques es conserven anonimitzades només en una zona aproximada de 10 km.</p><button className="pill finding-button-danger" onClick={() => { setDeleteError(null); setDeleteOpen(true); }} disabled={busy}>Eliminar definitivament el compte</button></section>
    <FindingDeleteDialog
      open={deleteOpen}
      busy={busy}
