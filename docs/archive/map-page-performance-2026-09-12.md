...
- A production browser CPU profile identified Gaussian raster interpolation as the main application-owned hot function. Reuse the separable horizontal and vertical exponential terms, retaining the circular cutoff, input order, Float32 accumulation, support fade, source grid and geographic anchoring. No prediction score or ecology changes.
- Alternating old/new Node 24 benchmarks with deterministic fixtures and 11 measured iterations gave median times of 26.20 → 17.60 ms (5,200 cells, 400×600), 31.72 → 20.84 ms (5,200 cells, 900×650), and 19.71 → 12.33 ms (1,000 cells, wider kernels). This is 33–37% less time in this calculation, not a whole-page speed claim. All output score and alpha Float32 values were identical in these fixtures. A tiny 0.5-pixel-kernel fixture rose from 0.089 to 0.100 ms.
...
Local reproducibility artifacts are under `artifacts/map-performance-2026-09-12/`: production CPU profile, PageSpeed text snapshots, benchmark source/results and verification logs. Maintained regression tests cover numerical parity, worker startup/module delivery, offline worker reuse, and species image priority.
