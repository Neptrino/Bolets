...
- Species-page habitat maps use a categorical dark-moss fill for static habitat cells whose centres fall inside a 10 km occurrence-support cell. Occurrence counts do not create a gradient or change the static forest/altitude/pH coverage: the tint can corroborate modeled habitat, while records outside compatible habitat remain a documented disagreement and unsampled cells are never excluded.
