...
The daily Story and weekend Reel allow two 60-second waits for the shared regional and local overview data, with a five-second pause before retrying data preparation (125 seconds maximum). A still-running read is reused rather than duplicated. They require publishable local readings and reject failed local hub loads before signing any artwork. The interactive sharing kit's shorter timeout may temporarily show broad regions while local data loads; that fallback must never become an automated publication. Only data preparation is retried, before contacting Buffer; publication calls are never automatically retried. After checking the channel date marker, the weekend publisher downloads the complete signed public MP4 with a 90-second limit and verifies its type, file header and declared length before submitting it to Buffer. This warms the completed render and fails closed when the video is unavailable; it never retries a create-post request. The daily invoking request allows five minutes and its service six minutes. The weekend request allows six minutes and its service seven minutes to include the video preparation budget.
