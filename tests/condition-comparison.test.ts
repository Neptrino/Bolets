...
describe("condition calculation presentation", () => {
  it("shows how high component responses produce the lower final cell score", () => {
    ...
    expect(html).toContain("Període: finestra del model · 26 dies");
    expect(html).toContain("El model compara la temperatura mitjana de 20 dies");
    expect(html).toContain("les gelades i la calor extrema s’apliquen per separat");
    ...
    expect(html).toContain("Drenatge de l’espècie");
    expect(html).toContain("sense dada de drenatge de la cel·la");
    expect(html).toContain("Normalitzada per textura entre punt de marciment");
    expect(html).toContain("es mostra com a context i no puntua separadament");
  });
});
