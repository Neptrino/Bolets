...
describe("zoom-adaptive spatial grid", () => {
  ...
  it("keeps the surface on 2.5 km without crossing access or timeline floors", () => {
    for (const floor of [250, 1000, 2500] as const) {
      expect(constrainGridSize(SMOOTHED_PREDICTION_GRID_SIZE_M, floor)).toBe(2500);
    }
    expect(constrainGridSize(SMOOTHED_PREDICTION_GRID_SIZE_M, 5000, 2500)).toBe(5000);
  });
