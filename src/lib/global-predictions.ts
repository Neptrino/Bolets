...
export async function getGlobalPredictionCells(
  ...
  // Coarse cells are coloured by the coverage-weighted mean of their 2.5 km
  // sectors and attributed to the best one, see coarse-cell-summary.ts;
...
async function summarisedGlobalChildren(bounds: SpatialBounds, gridSizeM: SummarisedGridSizeM) {
  ...
  return {
    buckets,
    summaries: summariseChildrenByParent(reads.flatMap((read) => read.cells), gridSizeM),
...
export async function getGlobalCellRanking(
  ...
  // the one the map attributes the cell to.
  const children = await summarisedGlobalChildren(bounds, gridSizeM);
  const child = children.summaries.get(cellId)?.best;
