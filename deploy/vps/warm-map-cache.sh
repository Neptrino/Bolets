...
for (const cache of ["warm-map-cache", "warm-local-guides"]) {
  const response = await fetch(`http://127.0.0.1:3000/api/internal/${cache}`, {
    method: "POST",
  ...
  if (result.status !== "unchanged") console.log(JSON.stringify({ cache, ...result }));
