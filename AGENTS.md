...
- Keep Meteocat XEMA station precipitation as a private hourly shadow stream with explicit half-hour completeness counts and a bounded rolling retention window. Treat gauges as observed ground truth that model archives cannot supply, interpolate with a strict station-distance cutoff instead of fabricating values for uncovered cells, and never let station rain enter production windows, the unified water model, or scores before a versioned station-versus-model validation promotes a single complete correction source.
